...
class SE_CPP_API Mathf {
   public:
    ...
    static Quaternion GetRot(const Matrix4& matrix) {
        return ::GetRot(matrix);
    }
    static void SetRot(Matrix4& matrix, const Quaternion& rot) {
        ::SetRot(matrix, rot);
    }
    static void SetPos(Matrix4& matrix, const Vector3& pos) {
        ::SetPos(matrix, pos);
    }
    static Vector3 GetPos(const Matrix4& matrix) {
    ...
    REFLECT_BEGIN(Mathf);
    REFLECT_METHOD(Abs);
    REFLECT_METHOD(SetRot);
    REFLECT_METHOD(GetRot);
    REFLECT_METHOD(SetPos);
    REFLECT_METHOD(GetPos);
    REFLECT_END();
};
...
inline void SerializeQuaternion(SerializationContext& context, const Quaternion& src) {
    context.SetType(SerializationContextType::Map);

    context.Child("x") << src.vector().x;
    context.Child("y") << src.vector().y;
    context.Child("z") << src.vector().z;
    context.Child("w") << src.scalar();
}

inline void DeserializeQuaternion(const SerializationContext& context, Quaternion& dst) {
    if (!context.IsDefined() || !context.IsSequence() || context.Size() < 4) {
        // v = defaultValue;
        return;
    }
    ...
    if (context.Child("z").IsDefined()) {
        context.Child("z") >> v.z;
    }
    if (context.Child("w").IsDefined()) {
        context.Child("w") >> s;
    }
    dst = Quaternion(s, v);
}
...
REFLECT_CUSTOM_EXT_BEGIN(Quaternion, SerializeQuaternion, DeserializeQuaternion);
REFLECT_METHOD(LookAt);
REFLECT_METHOD(scalar);
REFLECT_METHOD(vector);
REFLECT_CUSTOM_EXT_END();

REFLECT_CUSTOM_EXT_BEGIN(Matrix4, SerializeMatrix4, DeserializeMatrix4);
REFLECT_CUSTOM_EXT_END();